...
```bash
git submodule add git@github.com:integra-lib/work-queue.git external/integra/work-queue
```
...
```bash
git -C external/integra/work-queue fetch --tags
git -C external/integra/work-queue checkout v0.2.0
git add external/integra/work-queue && git commit -m "build: bump work-queue to v0.2.0"
```
